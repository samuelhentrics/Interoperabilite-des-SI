...
const pool = new Pool({
  ...
  database: process.env.DB_NAME || 'db_constructwagons',
});
...
const WEBHOOK_FROM = process.env.WEBHOOK_FROM || 'erp-constructwagons';
...
app.post('/api/demandes', express.text({ type: 'application/xml' }), async (req, res) => {
  ...
  // 2) Cas FRONT constructwagons : il envoie du JSON
  ...
  try {
    switch (event) {
      case 'add-demande': {
        ...
        if (!resp.ok) {
          return res.status(502).json({
            error: 'Failed to create demande in constructwagons ERP',
            status: resp.status,
            body: respText,
          });
        }

        return res.status(200).json({ message: 'Demande créée dans constructwagons', xmlSent: xml });
      }
      ...
      case 'update-demande': {
        ...
        if (!resp.ok) {
          return res.status(502).json({
            error: 'Failed to update demande in constructwagons ERP',
            status: resp.status,
            body: respText,
          });
        }

        return res.status(200).json({ message: 'Demande mise à jour dans constructwagons', xmlSent: xml });
      }
      ...
      case 'delete-demande': {
        ...
        if (!resp.ok) {
          return res.status(502).json({
            error: 'Failed to delete demande in constructwagons ERP',
            status: resp.status,
            body: respText,
          });
        }

        return res.status(200).json({ message: 'Demande supprimée dans constructwagons' });
      }
    ...
    console.error('💥 Erreur dans le handler /webhook constructwagons :', err);
    return res.status(500).json({ error: 'Internal error in constructwagons webhook' });
  }
});
...
async function subscribeToWebhook() {
    ...
    try {
        const who = process.env.WEBHOOK_WHO || 'erp-constructwagons';
...
app.listen(PORT, () => {
  console.log(`✅ back-constructwagons on http://0.0.0.0:${PORT}`);
